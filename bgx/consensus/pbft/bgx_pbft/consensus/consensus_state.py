...
class ConsensusState:
    ...
    def consensus_state_for_block_id(block_id,
        ...
        while True:
            ...
            if consensus_state is not None:
                LOGGER.debug("ConsensusState: FOUND CONSENSUS_STATE=%s for current_id=%s block=(%s)",consensus_state,current_id,block)
                break
            else:
                # save new state 
                LOGGER.debug("ConsensusState: NEW CONSENSUS_STATE for block_id=%s",block_id)
        ...
        if consensus_state is None:
            LOGGER.debug("ConsensusState: NEW CONSENSUS_STATE for block_id=%s",current_id)
            consensus_state = ConsensusState()
            consensus_state_store[block_id] = consensus_state
    ...
    PBFT part
    """
    def start_consensus(self,block,block_cache,state_view_factory,consensus_state_store):
        """
        Have got NewBlock message
        """
        self.next_step()
        consensus_state_store[block.previous_block_id] = self
        return True
        #block_str = int2hex(block.block_num)
        block_id = block.block_id.hex()
        
        if self is not None:
            if self.is_step_NotStarted:
                LOGGER.debug('ConsensusState: START CONSENSUS for block_id=%s step=%s mode=%s',block_id,self.step,self.mode)
                self.next_step()
                consensus_state_store[block_id] = self # save new state
                if node == 'leader':
                    # leader node - send prePrepare
                    self._send_pre_prepare(self,block)
                elif  node == 'plink' :
                    # just change step of consensus
                    LOGGER.debug('ConsensusState: PLINK node CONSENSUS step=%s',self.step)
                return True
            else:
                LOGGER.debug('ConsensusState: cant START CONSENSUS for block_id=%s (incorrect state) ',block_id)
        else:
            LOGGER.debug('ConsensusState: there is no CONSENSUS_STATE for block_id=%s',block_id)
            return False
