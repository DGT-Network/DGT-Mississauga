    ...
    def register_peer(self, connection_id, endpoint):
        ...
        Args:
            ...
            endpoint (str): The publically reachable endpoint of the new
                peer
        """
        LOGGER.debug("gossip:register_peer peers=%d",len(self._peers))
        with self._lock:
...
class ConnectionManager(InstrumentedThread):
    ...
    def run(self):
        ...
        LOGGER.debug("ConnectionManager:run has_chain_head=%s",has_chain_head)
        ...
        for endpoint in endpoints:
            ...
            if conn_id is not None:
                if not self._network.is_connection_handshake_complete(conn_id):
                    ...
                    # process yet.
                    LOGGER.debug("gossip:_get_peers_of_endpoints has not finished the authorization ")
                    continue
                    ...
                    if endpoint in self._temp_endpoints:
                        ...
                        LOGGER.debug("gossip:Endpoint %s is not yet authorized, do not request peers %d ",endpoint,len(peers))
                        continue
